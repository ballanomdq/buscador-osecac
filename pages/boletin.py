...
.badge-alta { background:#fee2e2; color:#991b1b; border:1px solid #fca5a5; border-radius:4px; padding:1px 6px; font-size:0.72rem; font-weight:600; }
.badge-media { background:#fef3c7; color:#92400e; border:1px solid #fcd34d; border-radius:4px; padding:1px 6px; font-size:0.72rem; font-weight:600; }
.badge-baja { background:#f3f4f6; color:#374151; border:1px solid #d1d5db; border-radius:4px; padding:1px 6px; font-size:0.72rem; font-weight:600; }
...
with c2:
    if st.button("🗑️ Limpiar boletines viejos"):
...
if st.session_state.get("mostrar_disponibles"):
    with st.expander("📥 Boletines disponibles", expanded=True):
        ...
        if disp:
            ...
            sel = st.selectbox("Elegí:", list(opciones.keys()))
...
with st.sidebar:
    ...
    filtro_confianza = st.multiselect("Nivel confianza", ["ALTA", "MEDIA", "BAJA"], default=["ALTA", "MEDIA", "BAJA"])
...
if filtro_confianza and len(filtro_confianza) < 3:
    df = df[df["nivel_confianza"].isin(filtro_confianza)]

if df.empty:
    st.info("No hay resultados para los filtros.")
    st.stop()

# ── Leyenda ───────────────────────────────────────────────────────────────────
st.markdown("""
<div style="display:flex; gap:12px; margin-bottom:8px; font-size:0.8rem;">
  <span class="badge-alta">🔴 ALTA — quiebra/concurso + nombre</span>
  <span class="badge-media">🟡 MEDIA — quiebra/concurso sin nombre</span>
  <span class="badge-baja">⚪ BAJA — solo localidad</span>
</div>
""", unsafe_allow_html=True)
...
# FUNCIÓN CORREGIDA - Maneja None en nombre y cuit
def resaltar_texto(texto, localidad, nombre, cuit):
    palabras = ["quiebra", "concurso", "subasta", "transferencia", localidad.lower()]
    if nombre and isinstance(nombre, str) and nombre not in ["Sin datos", "None", "", "Sin nombre"]:
    ...
    for p in palabras:
        if p:
            try:
                resultado = re.sub(rf'\b({re.escape(p)})\b', r'<span class="resaltado">\1</span>', resultado, flags=re.IGNORECASE)
            except:
                pass
    return resultado
...
def generar_html_impresion(row, numero, fecha_str):
    ...
    return f"""<html><head><meta charset="UTF-8"><title>Boletín N° {numero}</title>
    <style>body{{font-family:Arial;margin:40px}}.info{{padding:10px;background:{color};border-left:6px solid #888;margin-bottom:20px}}.edicto{{white-space:pre-wrap}}.resaltado{{background:#ffff99;font-weight:bold}}</style>
    <body><div class="info"><strong>Boletín Oficial PBA</strong><br>N° {numero} | Fecha: {fecha_str}<br>Localidad: {row['localidad']} | Tipo: {row.get('tipo_edicto','?')} | Página: {row.get('pagina','?')}<br>Sujeto: {nombre} | CUIT: {cuit}<br>Confianza: {nivel}</div><div class="edicto">{texto_r}</div></body></html>"""
        ...
        with col_a:
            with st.expander(titulo_bol, expanded=False):
                for _, row in grupo.iterrows():
                    ...
                    titulo_ed = f"{icono} {tipo} | {localidad} | {nombre_mostrar}" + (f" — {cuit}" if cuit else "") + f" (p.{pagina})"
                    ...
                    with st.expander(titulo_ed):
                        st.markdown(f"Nivel: {badge_confianza(nivel)}", unsafe_allow_html=True)
                        st.markdown("---")
                        ...
                        with cy:
                            if st.button("🗑️ Eliminar", key=f"del_btn_{row['id']}"):
                                ...
                                if st.session_state.get(ck):
                                    ...
                                    st.warning("Click otra vez")
                        with cz:
                            if st.button("🖨️ Imprimir", key=f"print_btn_{row['id']}"):
