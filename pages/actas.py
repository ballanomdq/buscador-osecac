...
<style>
/* === RESET Y FUENTES === */
html, body, [class*="css"] { font-size: 13px !important; font-family: 'Inter', system-ui, -apple-system, sans-serif; }
...
.app-header h3 { color: #f1f5f9; margin: 0; font-size: 1.1rem; font-weight: 500; letter-spacing: -0.2px; }
.app-header p { color: #64748b; margin: 0; font-size: 0.7rem; }
...
/* === CARTELES PROFESIONALES (estilo marca de agua) === */
.dashboard-grid {
    display: flex;
    gap: 0.8rem;
    margin-bottom: 1rem;
...
.stat-con-legajo .stat-value { color: #10b981; }
.stat-sin-legajo .stat-value { color: #f59e0b; }
.stat-pendiente .stat-value { color: #ef4444; }
.stat-mail .stat-value { color: #f97316; }
.stat-finalizado .stat-value { color: #10b981; }
...
.inspector-card .inspector-name {
    font-size: 0.7rem;
    font-weight: 600;
    color: #10b981;
    margin: 0 0 0.2rem 0;
}
.inspector-card .inspector-count {
    font-size: 1.2rem;
    font-weight: 700;
    color: #f1f5f9;
    margin: 0;
...
.inspector-card .inspector-legajo {
    font-size: 0.55rem;
    color: #64748b;
    margin: 0;
}
...
/* Filtros */
.filtro-titulo { font-size: 0.6rem; color: #64748b; margin-bottom: 0.1rem; letter-spacing: 0.3px; }
...
# ══════════════════════════════════════════════════════════════════
# TAB 2 — Editar Legajos y Vtos (DISEÑO PROFESIONAL)
# ══════════════════════════════════════════════════════════════════
with tab2:
    ...
    # ════════════════════════════════════════════════════════════════════════════
    # SECCIÓN 1: CONTEO DE REGISTROS (colapsable)
    # ════════════════════════════════════════════════════════════════════════════
    with st.expander("📊 CONTEO DE REGISTROS", expanded=False):
        ...
        with col2:
            st.markdown(f"""
            <div class="stat-card stat-con-legajo">
                <div class="stat-value">{con_legajo:,}</div>
                <div class="stat-label">CON LEGAJO</div>
                <div class="stat-icon">✅</div>
            </div>
            """, unsafe_allow_html=True)
        with col3:
            st.markdown(f"""
            <div class="stat-card stat-sin-legajo">
                <div class="stat-value">{sin_legajo_total:,}</div>
                <div class="stat-label">SIN LEGAJO</div>
                <div class="stat-icon">⚠️</div>
            </div>
            """, unsafe_allow_html=True)

    # ════════════════════════════════════════════════════════════════════════════
    # SECCIÓN 2: ESTADO DE REGISTROS (colapsable)
    # ════════════════════════════════════════════════════════════════════════════
    with st.expander("🔄 ESTADO DE REGISTROS", expanded=False):
        ...
        with col1:
            ...
            <div class="stat-card stat-pendiente">
                <div class="stat-value">{pendientes_sin_mail:,}</div>
                <div class="stat-label">PENDIENTES · SIN MAIL</div>
                <div class="stat-icon">📧</div>
            </div>
            """, unsafe_allow_html=True)
        with col2:
            st.markdown(f"""
            <div class="stat-card stat-mail">
                <div class="stat-value">{pendientes_con_mail:,}</div>
                <div class="stat-label">MAIL ENVIADO</div>
                <div class="stat-icon">📨</div>
            </div>
            """, unsafe_allow_html=True)
        with col3:
    ...
    # ════════════════════════════════════════════════════════════════════════════
    # SECCIÓN 3: EMPRESAS POR INSPECTOR (colapsable)
    # ════════════════════════════════════════════════════════════════════════════
    inspectores = supabase.table("inspectores").select("*").order("legajo").execute()
    if inspectores.data:
            ...
            for idx, ins in enumerate(inspectores.data):
                ...
                with cols[idx]:
                    ...
                    <div class="inspector-card">
                        <div class="inspector-name">{nombre_corto}</div>
                        <div class="inspector-count">{count}</div>
                        <div class="inspector-legajo">Legajo {ins['legajo']}</div>
                    </div>
                    """, unsafe_allow_html=True)
    ...
    with col_reset:
        if st.button("↺ Resetear filtros", use_container_width=True):
            for k in ['input_filtro_cuit','input_filtro_razon','filtro_localidad', 'filtro_mail','filtro_leg','filtro_calle_aproximacion','pagina_actual']:
...
with tab3:
    ...
    st.markdown("""
    <div style="background: #0f172a; padding: 0.5rem 1rem; border-radius: 10px; border-left: 3px solid #3b82f6; margin-bottom: 1rem;">
...
with tab4:
    st.markdown("""
    <div style="background: linear-gradient(135deg, #0f172a, #1e293b); border-radius: 12px; padding: 1rem; text-align: center; border: 1px solid #3b82f6;">
...
with tab5:
    st.markdown("""
    <div style="background: linear-gradient(135deg, #0f172a, #1e293b); border-radius: 12px; padding: 1rem; text-align: center; border: 1px solid #10b981;">
