...
# Conexión a Supabase (mismo proyecto que actas.py)
...
def crear_tablas():
    try:
        ...
        supabase.table("inspectores").select("id").limit(1).execute()
    except:
        st.info("Creando tabla 'inspectores'...")
        # La tabla se crea manualmente desde Supabase. Mostrar SQL.
        st.code("""
    ...
    try:
        # Tabla de zonas (calles por inspector)
        supabase.table("zonas_inspectores").select("id").limit(1).execute()
    except:
        st.info("Creando tabla 'zonas_inspectores'...")
        st.code("""
        CREATE TABLE zonas_inspectores (
            id SERIAL PRIMARY KEY,
            legajo TEXT NOT NULL,
            calle TEXT NOT NULL,
            lado TEXT CHECK (lado IN ('PAR', 'IMPAR', 'AMBOS')),
...
with tab1:
    ...
    if inspectores.data:
        df_inspectores = pd.DataFrame(inspectores.data)
        
        # Mostrar tabla
        for idx, row in df_inspectores.iterrows():
            ...
            with col3:
                if st.button("✏️", key=f"edit_insp_{row['id']}"):
                    st.session_state.editando_inspector = row.to_dict()
            with col4:
        ...
        st.info("No hay inspectores cargados. Agregá uno usando el formulario.")
...
with tab2:
    ...
    if not inspectores.data:
        ...
        with st.expander("➕ Agregar calle/zona", expanded=False):
            with st.form("form_nueva_zona"):
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    calle = st.text_input("Calle", placeholder="Ej: COLON")
                with col2:
                    lado = st.selectbox("Lado", options=["AMBOS", "PAR", "IMPAR"])
                with col3:
                    altura_desde = st.number_input("Altura desde", min_value=0, step=1, value=1)
                with col4:
                    altura_hasta = st.number_input("Altura hasta", min_value=0, step=1, value=9999)
                
                if st.form_submit_button("💾 Guardar zona", type="primary"):
                    if calle:
                        try:
                            ...
                            st.success(f"✅ Zona agregada: {calle.upper()} ({lado})")
                            st.rerun()
                        except Exception as e:
                            st.error(f"Error: {str(e)}")
                    else:
                        st.warning("Completá al menos la calle")
        
        # Listado de zonas del inspector
        st.markdown(f"### 📋 Calles asignadas a {inspector_seleccionado}")
        ...
        if zonas.data:
            ...
            for idx, row in df_zonas.iterrows():
                ...
                with col4:
                    if st.button("✏️", key=f"edit_zona_{row['id']}"):
                        st.session_state.editando_zona = row.to_dict()
                with col5:
            ...
            if st.session_state.get('editando_zona'):
                ...
                col_btn1, col_btn2 = st.columns(2)
                with col_btn1:
                    if st.button("💾 Guardar cambios zona"):
...
with tab3:
    ...
    # Verificar que hay inspectores y zonas cargadas
