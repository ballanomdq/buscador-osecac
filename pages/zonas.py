...
st.title("Mapa de Jurisdicciones - Inspectores Municipales")

# Diccionario maestro con las coordenadas exactas de las esquinas (Ordenadas para cerrar el polígono)
ZONAS_DATA = {
    "RODRIGUEZ": {
        "color": "red",
        "zonas": [
...
# Dibujar las zonas
for inspector, info in ZONAS_DATA.items():
    for zona in info["zonas"]:
        folium.Polygon(
            ...
            fill_opacity=0.4,
            weight=2,
            popup=f"<b>Inspector:</b> {inspector}<br><b>Zona:</b> {zona['nombre']}",
            tooltip=f"{inspector} - {zona['nombre']}"
        ).add_to(m)
