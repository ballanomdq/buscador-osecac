...
# TAB 3: CALLES - EDITOR CON FORMULARIOS POR FILA (CORREGIDO: permite altura 0)
with tab3:
    ...
    st.caption("⚠️ Para direcciones sin número (altura 0), podés poner **desde=0** y **hasta=0**")
    ...
    if not inspectores.data:
        ...
        if zonas.data:
                ...
                if st.session_state.get(f"editando_{zona['id']}"):
                    with st.expander(f"✏️ Editando: {zona['calle']}", expanded=True):
                        with st.form(key=f"form_edit_{zona['id']}"):
                            ...
                            with col_d:
                                nueva_desde = st.number_input("Altura desde", min_value=0, value=int(zona['altura_desde']), step=1)
                            with col_e:
                                nueva_hasta = st.number_input("Altura hasta", min_value=0, value=int(zona['altura_hasta']), step=1)
            ...
            with st.form("form_nueva_calle", clear_on_submit=True):
                ...
                with col4:
                    nueva_desde = st.number_input("Altura desde", min_value=0, value=1, step=1)
                with col5:
                    nueva_hasta = st.number_input("Altura hasta", min_value=0, value=9999, step=1)
