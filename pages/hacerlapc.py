...
def consultar_sisa_directo(dni):
    ...
    # ADN de la consulta extraído de tus capturas
    ...
    try:
        ...
        # 1. Buscamos el inicio de la lista de datos reales
        if '["ar.gob.msal' in text:
            # Cortamos el texto para quedarnos solo con la parte de los nombres
            start = text.find('[')
            end = text.rfind(']')
            if start != -1 and end != -1:
                data_segment = text[start+1:end]
                # Limpiamos las comillas y dividimos por coma
                partes = [p.strip().strip('"') for p in data_segment.split(',')]
                
                # 2. Buscamos el DNI en la lista limpia
                if str(dni) in partes:
                    idx = partes.index(str(dni))
                    # Patrón confirmado: DNI + 2 es OS, DNI + 3 es Nombre
                    return {
                        "DNI": dni,
        ...
        return {"DNI": dni, "Cobertura": "Error de Red", "Beneficiario": str(e), "Estado": "📡 Fallo"}
