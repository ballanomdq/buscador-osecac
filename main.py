                    ...
                    if edicion_habilitada:
                        with st.expander(f"📝 Editar fila {i}"):
                            ...
                            if st.button("Guardar Cambios", key=f"btn_{i}"):
                                try:
                                    # Asegúrate de tener esta función definida en tu código si planeas usarla
                                    if 'editar_celda_google_sheets' in globals() and editar_celda_google_sheets(url_u, i, c_edit, v_edit):
